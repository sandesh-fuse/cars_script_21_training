...
======================
Script 21 (cult + standard split) with quantile prediction:
  - For cult route: 5th/50th/95th percentile models (3 models)
  - For standard route: 5th/50th/95th percentile models (3 models)
  - At inference: route by cult flag, then predict all 3 quantiles
  - Quantile-crossing fix; empirical coverage check; optional Optuna + GPU

USAGE:
    Edit DATA_PATH below, then:
        python train_save_script21.py                        # CPU, defaults (fast)
        python train_save_script21.py --gpu                  # GPU, defaults
        python train_save_script21.py --retune --n_trials 50 # CPU + Optuna (~9 hr)
        python train_save_script21.py --retune --gpu         # GPU + Optuna (~2 hr if GPU works)

OUTPUTS (./artifacts/script21/):
