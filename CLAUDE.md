...
Key files:
- [preprocessor.py](preprocessor.py) (top-level, ~1100 lines) — the single most shared module: feature engineering, macro data tables (CPI/Manheim/auto-loan), `build_monthly_series()` interpolation, `compute_cult_flag()`, `cpi_ratio_arr()`/`deflate_pred()`. Imported by both inference pipelines and both training scripts.
