"""
run_vendor_ablation.py
========================
Ablation-tests the 3 vendor-region-effect features ported from main's
commit 2de22bd (vendor-lookup piece only -- see VENDOR_FEATURE_COLS in
preprocessor.py and build_vendor_zip_lookup() for the leakage-safe
construction: vendor_id is target-encoded, then backed off to a 3-digit
zip-prefix level so vendor identity never has to be available at
inference, only zip code):

  vendor_region_effect, vendor_region_effect_x_age,
  vendor_region_effect_x_mileage_bucket

Tests each ONE AT A TIME against a shared baseline (--enable-vendor-
features none) -- not cumulative -- via subprocess calls to
train_save_script21.py's --enable-vendor-features flag, then evaluates
every run the same way train_save_script21.py always does
(test_metrics.json via evaluate_predictions.evaluate()) and renders a
combined overall + per-tier comparison table.

NOTE: this feature requires a 'vendor_id' column in the raw --data CSV.
If it's absent, VENDOR_ZIP_LOOKUP ends up empty and every group
(including "all") is silently identical to the baseline -- check each
run's log for "Vendor-region effect lookup: skipped" before trusting a
null result.

USAGE:
    python run_vendor_ablation.py                        # baseline + all 3, one at a time
    python run_vendor_ablation.py --only vendor_region_effect
    python run_vendor_ablation.py --only baseline,vendor_region_effect
    python run_vendor_ablation.py --include-all-combined  # + a 4th group: all 3 together
    python run_vendor_ablation.py --gpu
"""
import os
import sys
import json
...
from preprocessor import VENDOR_FEATURE_COLS
...
def build_groups(include_all_combined):
    ...
    groups += [(name, name) for name in VENDOR_FEATURE_COLS]
...
def run_group(name, enable_value, args):
    ...
    cmd = [
        ...
        "--cult", args.cult,
        "--out", out_dir,
        "--enable-vendor-features", enable_value,
    ]
    ...
    if args.enable_condition_make_features != "all":
        cmd += ["--enable-condition-make-features", args.enable_condition_make_features]
    if args.gpu:
        cmd.append("--gpu")
    if args.use_dataone:
        cmd.append("--use-dataone")
    print(f"\n{'=' * 100}\nGROUP {name}: --enable-vendor-features {enable_value}\n{'=' * 100}")
    print("  $", " ".join(cmd))
    t0 = time.time()
    log_path = f"vendor_ablation_{name}.log"
    ...
    with open(log_path) as logf:
        log_text = logf.read()
    if "Vendor-region effect lookup: skipped" in log_text:
        print(f"  WARNING: vendor_id not found in {args.data} -- this run has an EMPTY "
              f"vendor lookup and will be identical to baseline regardless of "
              f"--enable-vendor-features. Results below are not meaningful.")
...
def render_markdown(df, tier_order):
    lines = ["# Vendor-region-effect feature ablation results\n"]
    lines.append(f"`{BASELINE_NAME}` = --enable-vendor-features none (pre-change behavior). "
                  "Each other group enables exactly ONE of the 3 features "
                  "against that same baseline (not cumulative).\n")
    lines.append("Ported from main's commit 2de22bd (vendor-lookup piece only). "
                  "`$0-200`/`$200-500`/`$500-1K`/`$1K-2.5K` print first (most volume); "
                  "`$2.5K+` kept for reference.\n")
...
def main():
    ...
    out_md = "vendor_ablation_results.md"
