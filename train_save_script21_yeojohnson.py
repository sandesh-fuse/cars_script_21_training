"""
train_save_script21_yeojohnson.py
==================================
Experimental variant of train_save_script21.py (branched from the "515 MAE"
checkpoint, commit 2406a7a) that replaces the log1p/expm1 target transform
with a per-route Yeo-Johnson power transform (sklearn PowerTransformer),
fit on the CPI-adjusted training target and inverse-transformed at test
time. Everything else (quantile architecture, CPI deflation, routing,
hyperparameters) is unchanged from the checkpoint. See Baseline1.txt for
the checkpoint's $516.73 test MAE to compare against.

NOTE: --save-shap / --save-shap-global are disabled in this variant.
shap_dollar_helper.py / _shap_export_helper.py hardcode np.expm1() to
convert model output to dollars, which is the WRONG inverse transform
under Yeo-Johnson and would silently produce incorrect dollar figures.

USAGE:
    Edit DATA_PATH below, then:
        python train_save_script21_yeojohnson.py                        # CPU, defaults (fast)
        python train_save_script21_yeojohnson.py --gpu                  # GPU, defaults
        python train_save_script21_yeojohnson.py --retune --n_trials 50 # CPU + Optuna (~9 hr)
        python train_save_script21_yeojohnson.py --retune --gpu         # GPU + Optuna (~2 hr if GPU works)

OUTPUTS (./artifacts/script21_yeojohnson/):
    preprocessor_cult.joblib, preprocessor_standard.joblib
    target_transform_cult.joblib, target_transform_standard.joblib
    model_cult_{q05,q50,q95}.json, model_standard_{q05,q50,q95}.json
...
ARTIFACTS_DIR    = "./artifacts/script21_yeojohnson"
...
# OPTUNA — per quantile
# ============================================================
def tune_quantile(name, X_tr, y_tr_adj, X_va, y_va_adj, monotone, quantile_alpha, n_trials, pt):
    import optuna
    ...
    def objective(trial):
        ...
        m.fit(X_tr, pt.transform(y_tr_adj.reshape(-1, 1)).ravel(),
              eval_set=[(X_va, pt.transform(y_va_adj.reshape(-1, 1)).ravel())], verbose=False)
        pred_va = pt.inverse_transform(m.predict(X_va).reshape(-1, 1)).ravel()
...
def train_subset_with_quantiles(name, config, default_params, train_subset, test_subset,
    ...
    # Fit the Yeo-Johnson transform once on the full (CPI-adjusted) training
    # target, before the early-stopping split, so both the es_tr/es_va slices
    # and the eventual test-time inverse_transform share one consistent
    # lambda. No test leakage: y_train_adj never includes test-set rows.
    pt = PowerTransformer(method='yeo-johnson', standardize=False)
    pt.fit(y_train_adj.reshape(-1, 1))
    ...
    for q, qlab in zip(QUANTILES, QUANTILE_LABELS):
        ...
        model.fit(X_es_tr, pt.transform(y_adj_tr.reshape(-1, 1)).ravel(),
                  eval_set=[(X_es_va, pt.transform(y_adj_va.reshape(-1, 1)).ravel())], verbose=False)

        pred_adj = np.clip(pt.inverse_transform(model.predict(X_test).reshape(-1, 1)).ravel(), 1, None)
    ...
    return pre, models, preds_test, best_params_by_q, pt
...
def main():
    ...
    parser.add_argument("--save-shap", action='store_true',
                        help="DISABLED in this variant: shap_dollar_helper.py/_shap_export_helper.py "
                             "hardcode np.expm1() to convert model output to dollars, which is the "
                             "wrong inverse transform under Yeo-Johnson. Passing this flag errors out.")
    parser.add_argument("--save-shap-global", action='store_true',
                        help="DISABLED in this variant: same np.expm1()-hardcoding reason as "
                             "--save-shap. Passing this flag errors out.")
    ...
    if args.save_shap or args.save_shap_global:
        parser.error(
            "--save-shap/--save-shap-global are disabled in this variant: "
            "shap_dollar_helper.py and _shap_export_helper.py hardcode np.expm1() "
            "to convert model output into dollars, which is the wrong inverse "
            "transform for a Yeo-Johnson-transformed target and would silently "
            "produce incorrect dollar figures."
        )
    ...
    pre_cult, models_cult, preds_cult, params_cult, pt_cult = train_subset_with_quantiles(
    ...
    pre_std, models_std, preds_std, params_std, pt_std = train_subset_with_quantiles(
    ...
    # shap_dollar_helper.py hardcodes np.expm1() to convert the model's raw
    # output into dollars; under Yeo-Johnson that is the wrong inverse
    # transform and would silently print incorrect dollar figures rather
    # than erroring. Fixing that helper is out of scope here (shared by the
    # live-serving code path too), so we skip it entirely for this variant.
    print("\n--- SHAP sanity check SKIPPED (yeojohnson target transform is "
          "incompatible with shap_dollar_helper's hardcoded expm1 dollar "
          "conversion) ---")
    ...
    joblib.dump(pt_cult, os.path.join(args.out, "target_transform_cult.joblib"))
    joblib.dump(pt_std,  os.path.join(args.out, "target_transform_standard.joblib"))
    for qlab in QUANTILE_LABELS:
    ...
    if len(train_cult_raw) > 0:
        ...
        for qlab in QUANTILE_LABELS:
            pred_adj = np.clip(pt_cult.inverse_transform(
                models_cult[qlab].predict(X_train_cult).reshape(-1, 1)).ravel(), 1, None)
    ...
    if len(train_std_raw) > 0:
        ...
        for qlab in QUANTILE_LABELS:
            pred_adj = np.clip(pt_std.inverse_transform(
                models_std[qlab].predict(X_train_std).reshape(-1, 1)).ravel(), 1, None)
