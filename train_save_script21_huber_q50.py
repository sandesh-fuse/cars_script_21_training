"""
train_save_script21_huber_q50.py
=================================
Experimental variant of train_save_script21.py (branched from the "515 MAE"
checkpoint, commit 2406a7a) that swaps ONLY the q50 (median) model's
training objective from XGBoost's quantile loss (reg:quantileerror,
quantile_alpha=0.5) to XGBoost's Huber loss (reg:pseudohubererror). q05/q95
stay exactly as the checkpoint (reg:quantileerror), since they drive the
90% CI / coverage logic. Target transform (log1p/expm1) and CPI deflation
...
OUTPUTS (./artifacts/script21_huber_q50/):
...
ARTIFACTS_DIR    = "./artifacts/script21_huber_q50"
TRAIN_CUTOFF     = "2018-01-01"
HUBER_SLOPE      = 1.0   # XGBoost's own default for reg:pseudohubererror; override via --huber-slope
SEED             = 42
...
def tune_quantile(name, X_tr, y_tr_adj, X_va, y_va_adj, monotone, quantile_alpha, n_trials,
    ...
    # pinball_loss(y, p, 0.5) == 0.5 * MAE(y, p) exactly, for any p — so the
    # tuning objective below is already the right thing to minimize whether
    # q50 is trained with quantile loss or Huber loss; only the trial model's
    # construction needs to branch.
    use_huber = (quantile_alpha == 0.5)
    ...
    for q, qlab in zip(QUANTILES, QUANTILE_LABELS):
        ...
        if args.retune:
            params = tune_quantile(
                ...
                monotone, q, args.n_trials, huber_slope=args.huber_slope)
        else:
...
def main():
    ...
    parser.add_argument("--huber-slope", type=float, default=HUBER_SLOPE,
                        help="XGBoost reg:pseudohubererror huber_slope for the q50 model "
                             f"(default {HUBER_SLOPE}, XGBoost's own default). Only applies to q50 — "
                             "q05/q95 remain reg:quantileerror.")
    ...
    parser.add_argument("--save-shap", action='store_true',
                        help="Compute and save per-row SHAP attributions for every train + "
                             ...
                             "test_shap_raw (each as parquet AND csv). For script21, "
                             "attributions are computed against the routed model "
                             "(cult/standard). Also writes the global importance summary "
                             "(same as --save-shap-global). Expensive: ~45-60 min CPU for "
                             "full train, multi-GB output.")
    parser.add_argument("--save-shap-global", action='store_true',
                        help="Compute and save GLOBAL SHAP feature importance only (small CSVs, "
                             "one row per feature, per route). For script21 the files are "
                             "split per route: train_shap_global_importance_cult.csv and "
                             "_standard.csv because cult and standard use different feature "
                             "spaces. Cheap: a few minutes on CPU. Implied by --save-shap.")
    ...
    print("\n--- SHAP sanity check (median models) ---")
    if len(test_cult_raw) >= 3:
        X_check_cult = pre_cult.transform(test_cult_raw.head(min(5, len(test_cult_raw))))
        shap_check_cult = sanity_check_shap(
            models_cult["q50"], X_check_cult, pre_cult.feature_cols_, label="CULT-q50")
    else:
        shap_check_cult = None
        print("  Skipped cult SHAP check (insufficient cult test rows)")
    X_check_std = pre_std.transform(test_std_raw.head(5))
    shap_check_std = sanity_check_shap(
        models_std["q50"], X_check_std, pre_std.feature_cols_, label="STANDARD-q50")
